// Fill out your copyright notice in the Description page of Project Settings.

#pragma once

#include "CoreMinimal.h"
#include "PEItemUseable.h"
#include "PEItemThrowable.generated.h"

class UPEAttackBaseComponent;

UCLASS()
class PROJECTESCAPE_API APEItemThrowable : public APEItemUseable
{
	GENERATED_BODY()

	/* Lifecycle 관련 섹션 */
public:
	APEItemThrowable();

protected:
	virtual void BeginPlay() override;

	/* Interact 관련 섹션 */
public:
	virtual void Interact(class AActor* Interactor) override;
	
	/* IPEUseable 인터페이스 선언 */
public:
	virtual void DoPrimaryAction(AActor* Holder) override;
	virtual void CompletePrimaryAction(AActor* Holder) override;
	virtual void DoSecondaryAction(AActor* Holder) override;
	virtual void DoTertiaryAction(AActor* Holder) override;
	
	/* Granade 관련 섹션 */
protected:
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Weapon|Granade")
	TObjectPtr<UPEAttackBaseComponent> AttackComponent;
	
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Weapon|Granade")
	float ThrowingReadyTime = 0.3f; // 그레네이드 던지기 준비 시간

	FTimerHandle ThrowingReadyTimerHandle;

	bool bIsFiring = false;
	bool bIsThrowingReady = false;
	bool bIsReleasedButton = false;

	// 투척 준비 완료 함수
	UFUNCTION()
	void ThrowingReady(AActor* Holder);

	UFUNCTION()
	void PerformThrowing(AActor* Holder);
};
